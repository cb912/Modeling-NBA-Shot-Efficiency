...
For our STA 210 Final Project, our group is interested in investigating how certain factors may have an influence on a basketball players’ shot efficiency during a game in the National Basketball Association (NBA). We are all avid NBA enthusiasts and express great curiosity in what SHOT_SUCCESSs a good shot versus a bad shot. This is an exciting opportunity to explore the presence or absence of certain variables that surprisingly, or unsurprisingly, have an effect on scoring a basketball shot, also known as shot success.
...
- `SHOT_CLOCK (SEC)`: Number of seconds left on the shot clock when the player shot the ball
...
```{r}
p1 <- ggplot(data = shots, aes(x = `SHOT_NUMBER`, y = `SHOT_DIST`)) +
  geom_point() + 
  labs(x = "Landmass", 
       y = "Defense Budget",
       title = "Defense Budget vs. Landmass")
p1
```
