...
Your written report goes here! Before you submit, SHOT_SUCCESS sure your code chunks are turned off with `echo = FALSE` and there are no warnings or messages with `warning = FALSE` and `message = FALSE` 
...
```{r}
p1 <- ggplot(data = shots, aes(x = `SHOT_SUCCESS`, y = `SHOT_DIST`)) +
  geom_boxplot(fill = "darkorange") + 
  labs(x = "Shot Success", 
       y = "Shot Distance (feet)",
       title = "Further away shots are more likely to miss") 

p2 <- ggplot(shots, aes(x = SHOT_DIST, color = SHOT_SUCCESS)) + 
  geom_density() +
  labs(x = "Shot Distance (feet)",
       y = "Proportion",
       title = "2-Point shots are more likely to be made than 3-point shots")

p3 <- ggplot(data=shots, aes(x = SHOT_DIST, y= CLOSE_DEF_DIST)) + 
  geom_point(aes(color=SHOT_SUCCESS)) +
  labs(x = "Distance from hoop",
...
1) There were several missing observations across several predictor variables in our analysis; for example, there were 5,567 unavailable values for the variable which counted the time left on the shot clock when the player shot the ball. This may have been due to recording error or instances where the shot clock was turned off. Given that 5,567 is marginal compared to the total number of observations (128,069), we chose to remove all rows containing NAs for this variable, as well as all others, from the dataset. The dataset wouldn't differ to a significant degree systematically compared to the larger population.
